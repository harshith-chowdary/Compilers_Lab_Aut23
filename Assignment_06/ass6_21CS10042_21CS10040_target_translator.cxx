...
void loadParam(string paramName, int paramNum) {
    ...
    if(type == SymbolType::ARRAY) {
        ...
        movIns = "movl";
        assemblyFile << "\t\t\t\t\t\t\t\t#" << paramName << " 'int' " << type << endl;
    } else if (size == 8) {
        movIns = "movq";
        assemblyFile << "\t\t\t\t\t\t\t\t#" << paramName << " 'flt' " << type << endl;
    }
...
void storeParam(string paramName, int paramNum) {
    ...
    if(type == SymbolType::ARRAY) {
        ...
        movIns = "movq";
        assemblyFile << "\t\t\t\t\t\t\t\t#" << paramName << " 'flt\ptr' " << type << endl;
    }
    string reg = getReg(paramName, paramNum, size);
    assemblyFile << "\t" << setw(8) << movIns << getStackLoc(paramName) << ", " << reg << endl;
}

int32_t floatToIEEE754(string arg) {
    union {
        float f;
        int32_t i;
    } ieee754;
    ...
    for(auto &quad:quadArray){
        if(quad->op == "="){
            ...
            if(isdigit(arg1[0])){
                ...
                if(f){
                    assemblyFile << ".LCFltConst" << f_const_count++ << ":" << endl;
                    assemblyFile << "\t.long\t" << floatToIEEE754(arg1) << endl;
                    // assemblyFile << "\t.float\t" << arg1 << endl;
                    assemblyFile << "\t.align 4" << endl;
                }
            }
        }
    }
            ...
            if(inTextSpace) {
                ...
                if(op == "=") {
                    ...
                    if(isdigit(arg1[0])) {
                        ...
                        if(f) assemblyFile << "\t" << setw(8) << "movl" << "$" << arg1 << ", " << getStackLoc(result) << endl;
                        else {
                            // assemblyFile << "\t" << setw(8) << "movq" << "$" << arg1 << ", " << getStackLoc(result) << endl;
                            // assemblyFile << "\t" << setw(8) << "movq" << "__?float64?__(" << arg1 << "), " << getStackLoc(result) << endl;

                            assemblyFile << "\t" << setw(8) << "movss" << ".LCFltConst" << c_f_const_count++ << "(%rip), %xmm0" << endl;
                            assemblyFile << "\t" << setw(8) << "movss" << "%xmm0, " << getStackLoc(result) << endl;
                            // assemblyFile << "\t" << setw(8) << "movq" << "$" << floatToIEEE754(arg1) << ", " << getStackLoc(result) << endl;
                        }
                        // __?float64?__(7.25)
                    } else if(arg1[0] == '\''){
                        // character constant
                        ...
                        int tar = currentTable->lookup(result)->size;
                        if(tar == 1) {
                            ...
                            if(sz == 8){
                                assemblyFile << "\t" << setw(8) << "movss" << getStackLoc(arg1) << ", %xmm0" << endl;
                                assemblyFile << "\t" << setw(8) << "cvtss2sil" << " %xmm0, " << "%eax" << endl;
                            ...
                            if(sz == 4){
                                assemblyFile << "\t" << setw(8) << "cvtsi2ssl " << getStackLoc(arg1) << ", " << "%xmm0" << endl;
                                assemblyFile << "\t" << setw(8) << "movss" << "%xmm0, " << getStackLoc(result) << endl;
                            }
                            else{
                    ...
                    SymbolType::typeEnum type = currentTable->lookup(arg)->type->type;

                    if(type == 3) assemblyFile << "\t" << setw(8) << "movss" << getStackLoc(arg) << ", %xmm0" << endl;
                    ...
                    if(result == arg1) {
                        ...
                        if(symbol->type->type == 3){
                            assemblyFile << "\t" << setw(8) << "movss" << getStackLoc(arg1) << ", " << "%xmm0" << endl;
                            assemblyFile << "\t" << setw(8) << "addss" << getStackLoc(arg2) << ", " << "%xmm0" << endl;
                            assemblyFile << "\t" << setw(8) << "movss" << "%xmm0" << ", " << getStackLoc(result) << endl;
                        } else {
                            assemblyFile << "\t" << setw(8) << "movl" << getStackLoc(arg1) << ", " << "%eax" << endl;
                    ...
                    if(result == arg1) {
                        ...
                        if(symbol->type->type == 3){
                            assemblyFile << "\t" << setw(8) << "movss" << getStackLoc(arg1) << ", " << "%xmm0" << endl;
                            assemblyFile << "\t" << setw(8) << "subss" << getStackLoc(arg2) << ", " << "%xmm0" << endl;
                            assemblyFile << "\t" << setw(8) << "movss" << "%xmm0" << ", " << getStackLoc(result) << endl;
                        } else {
                            assemblyFile << "\t" << setw(8) << "movl" << getStackLoc(arg1) << ", " << "%eax" << endl;
                    ...
                    if(symbol->type->type == 3){
                        assemblyFile << "\t" << setw(8) << "movss" << getStackLoc(arg1) << ", " << "%xmm0" << endl;
                        assemblyFile << "\t" << setw(8) << "mulss" << getStackLoc(arg2) << ", " << "%xmm0" << endl;
                        assemblyFile << "\t" << setw(8) << "movss" << "%xmm0" << ", " << getStackLoc(result) << endl;
                    } else {
                        assemblyFile << "\t" << setw(8) << "movl" << getStackLoc(arg1) << ", " << "%eax" << endl;
                        if(isdigit(arg2[0])) {
                    ...
                    if(symbol->type->type == 3){
                        assemblyFile << "\t" << setw(8) << "movss" << getStackLoc(arg1) << ", " << "%xmm0" << endl;
                        assemblyFile << "\t" << setw(8) << "divss" << getStackLoc(arg2) << ", " << "%xmm0" << endl;
                        assemblyFile << "\t" << setw(8) << "movss" << "%xmm0" << ", " << getStackLoc(result) << endl;
                    } else {
                        assemblyFile << "\t" << setw(8) << "movl" << getStackLoc(arg1) << ", " << "%eax" << endl;
                    ...
                    if(symbol->type->type == 3){
                        assemblyFile << "\t" << setw(8) << "movss" << getStackLoc(arg1) << ", " << "%xmm0" << endl;
                        assemblyFile << "\t" << setw(8) << "xorps" << "%xmm1, %xmm1" << endl;
                        assemblyFile << "\t" << setw(8) << "subss" << "%xmm0, %xmm1" << endl;
                        assemblyFile << "\t" << setw(8) << "movss" << "%xmm1" << ", " << getStackLoc(result) << endl;
                    } else {
                        assemblyFile << "\t" << setw(8) << "movl" << getStackLoc(arg1) << ", " << "%eax" << endl;
                ...
                if(currentSymbol->category == Symbol::TEMPORARY) {
                    ...
                    } else if(currentSymbol->type->type == SymbolType::FLOAT) {
                        assemblyFile << "\t" << setw(8) << ".globl" << currentSymbol->name << endl;
                        assemblyFile << "\t" << setw(8) << ".data" << endl;
