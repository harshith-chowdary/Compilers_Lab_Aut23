...
// Defining activation record class
ActivationRecord::ActivationRecord() : totalDisplacement(0), displacement(map<string, int>()) {}  // start with displacement of 0
...
Symbol *Symbol::convert(SymbolType::typeEnum type_)
{
    ...
    if ((this->type)->type == SymbolType::typeEnum::FLOAT)
    {
        ...
        if (type_ == SymbolType::typeEnum::INT)
        {
            ...
            // emit("=", fin_->name, "FloatToInt(" + this->name + ")");
            emit("=", fin_->name, this->name);
            return fin_;
        }
        ...
        if (type_ == SymbolType::typeEnum::CHAR)
        {
            ...
            Symbol *fin_ = gentemp(type_);
            // emit("=", fin_->name, "FloatToChar(" + this->name + ")");
            emit("=", fin_->name, this->name);
            return fin_;
        }
    ...
    if ((this->type)->type == SymbolType::typeEnum::INT)
    {
        ...
        if (type_ == SymbolType::typeEnum::FLOAT)
        {
            ...
            Symbol *fin_ = gentemp(type_);
            // emit("=", fin_->name, "IntToFloat(" + this->name + ")");
            emit("=", fin_->name, this->name);
            return fin_;
        }
        ...
        if (type_ == SymbolType::typeEnum::CHAR)
        {
            // generate symbol of new type char
            Symbol *fin_ = gentemp(type_);
            // emit("=", fin_->name, "IntToChar(" + this->name + ")");
            emit("=", fin_->name, this->name);
            return fin_;
        }
    ...
    if ((this->type)->type == SymbolType::typeEnum::CHAR)
    {
        ...
        if (type_ == SymbolType::typeEnum::INT)
        {
            // generate symbol of new type int
            Symbol *fin_ = gentemp(type_);
            // emit("=", fin_->name, "CharToInt(" + this->name + ")");
            emit("=", fin_->name, this->name);
            return fin_;
        }
        ...
        if (type_ == SymbolType::typeEnum::FLOAT)
        {
            // generate symbol of new type float
            Symbol *fin_ = gentemp(type_);
            // emit("=", fin_->name, "CharToFloat(" + this->name + ")");
            emit("=", fin_->name, this->name);
            return fin_;
        }

        // return orignal symbol if the final type is not int or float
        return this;
    }
